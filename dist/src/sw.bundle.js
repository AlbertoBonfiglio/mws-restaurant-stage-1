...
var cachedURLS = ['/index.html', '/restaurant.html', //'/data/',
'/css/', '/img/', '/js/', 'sw.bundle.js'];
...
var matchCb = function matchCb(_ref) {
  var url = _ref.url,
      event = _ref.event;
  return url.pathname === '/restaurants';
};

var handlerCb = function handlerCb(_ref2) {
  var url = _ref2.url,
      event = _ref2.event,
      params = _ref2.params;
  return fetch(event.request).then(function (response) {
    trest(response.clone());
    return response; //let y = new Response();
    //y.status = 666;
    //y.statusText = 'suca';
    //return y;
  })["catch"](function (err) {
    var _parseQueryString = parseQueryString(url.search),
        query = _parseQueryString.query,
        variables = _parseQueryString.variables;

    console.log(query); // read data from indexed db here
  });
};
